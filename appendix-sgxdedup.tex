...
\begin{lstlisting}[style=json,caption={\sysnameS 原型系统运行参数},label={lst:sgxdedup--config}]
{
    "ChunkerConfig": {
        ...
        "_minChunkSize": 4096, 
        // The smallest chunk size in variable size chunking, Uint: Byte (Maximum size 16KiB)
        "_avgChunkSize": 8192, 
        // The average chunk size in variable size chunking and chunk size in fixed size chunking, Uint: Byte (Maximum size 16KiB)
        "_maxChunkSize": 16384, 
        // The biggest chunk size in variable size chunking, Uint: Byte (Maximum size 16KiB)
        "_slidingWinSize": 256, 
        // The sliding window size in variable size chunking, Uint: MiB
        "_ReadSize": 256 
        // System read input file size every I/O operation, Uint: MiB
    },
    "KeyServerConfig": {
        "_keyBatchSize": 4096, 
    ...
    "SPConfig": {
        ...
        // Maximum space for one-time persistent chunk storage, Uint: Byte (Maximum size 8MiB)
    },
    "pow": {
        "_quoteType": 0, 
