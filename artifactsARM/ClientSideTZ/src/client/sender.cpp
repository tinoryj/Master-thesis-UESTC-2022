...
bool Sender::sendPoWKeyGenMessage(u_char* powKey, u_char* powKeySig)
{
    NetworkHeadStruct_t requestBody, respondBody;
    requestBody.clientID = clientID_;
    requestBody.messageType = POW_KEY_GEN;
    requestBody.dataSize = 288;
    int sendSize = sizeof(NetworkHeadStruct_t) + requestBody.dataSize;
    char requestBuffer[sendSize];
    memcpy(requestBuffer, &requestBody, sizeof(NetworkHeadStruct_t));
    memcpy(requestBuffer + sizeof(NetworkHeadStruct_t), powKey, 256);
    memcpy(requestBuffer + sizeof(NetworkHeadStruct_t) + 256, powKeySig, 32);
    char respondBuffer[sizeof(NetworkHeadStruct_t)];
    int recvSize = 0;
    if (!this->sendDataPow(requestBuffer, sendSize, respondBuffer, recvSize)) {
        cerr << "Sender : peer closed, set log out error" << endl;
        ...
        if (respondBody.messageType = SUCCESS) {
            cerr << "Sender : PoW key exchange done" << endl;
            return true;
        } else {
            cerr << "Sender : PoW key exchange error" << endl;
            return false;
        }
    }
}
...
bool Sender::sendEnclaveSignedHash(u_char* clientMac, u_char* hashList, int requestNumber, u_char* respond, int& status)
{
    ...
    int sendSize = sizeof(NetworkHeadStruct_t) + sizeof(uint8_t) * 32 + requestNumber * SYSTEM_CIPHER_SIZE;
    requestBody.dataSize = sizeof(uint8_t) * 32 + requestNumber * SYSTEM_CIPHER_SIZE;
    char requestBuffer[sendSize];
    memcpy(requestBuffer, &requestBody, sizeof(NetworkHeadStruct_t));
    memcpy(requestBuffer + sizeof(NetworkHeadStruct_t), clientMac, sizeof(uint8_t) * 32);
    memcpy(requestBuffer + sizeof(NetworkHeadStruct_t) + sizeof(uint8_t) * 32, hashList, requestNumber * SYSTEM_CIPHER_SIZE);
