    ...
    while (true) {
        ...
        if (extractMQ(tempChunk)) {
            if (tempChunk.dataType == DATA_TYPE_RECIPE) {
                ...
                cerr << "Feature key = " << endl;
                PRINT_BYTE_ARRAY_ENCODER(stderr, tempChunk.chunk.encryptKeyFeature, SYSTEM_CIPHER_SIZE);
                cerr << "MLE key = " << endl;
                PRINT_BYTE_ARRAY_ENCODER(stderr, tempChunk.chunk.encryptKeyMLE, SYSTEM_CIPHER_SIZE);
#endif
                u_char ciphertext[tempChunk.chunk.logicDataSize];
                cryptoObj_->encryptWithKey(tempChunk.chunk.logicData, tempChunk.chunk.logicDataSize, tempChunk.chunk.encryptKeyMLE, ciphertext);
                // if (tempChunk.chunk.logicDataSize <= PREFIX_LENGTH) {
                ...
                // } else {
                //     cryptoObj_->encryptWithKey(tempChunk.chunk.logicData, PREFIX_LENGTH, tempChunk.chunk.encryptKeyFeature, ciphertext);
                //     cryptoObj_->encryptWithKey(tempChunk.chunk.logicData + PREFIX_LENGTH, tempChunk.chunk.logicDataSize - PREFIX_LENGTH, tempChunk.chunk.encryptKeyMLE, ciphertext + PREFIX_LENGTH);
                // }
#if SYSTEM_BREAK_DOWN == 1
