...
Fingerprinter::Fingerprinter(KeyClient* keyClientObjTemp)
{
    ...
    initFeatureGen();
    keySeedFeatureNumber_ = config.getKeySeedFeatureNumber();
    keySeedSuperFeatureNumber_ = config.getKeySeedSuperFeatureNumber();
    featureNumberPerSuperFeature_ = keySeedFeatureNumber_ / keySeedSuperFeatureNumber_;
        ...
        if (extractMQ(tempChunk)) {
            if (tempChunk.dataType == DATA_TYPE_RECIPE) {
                ...
                if (generateKeySeedStatus) {
                    memset(tempChunk.chunk.chunkHash, 0, SYSTEM_CIPHER_SIZE);
...
bool Fingerprinter::extractSFList(uint64_t fpList[], int chunkSize, vector<string>& superFeatureList)
{
    ...
    u_char featuresBuffer[SYSTEM_CIPHER_SIZE];
    memset(featuresBuffer, 0, SYSTEM_CIPHER_SIZE);
    for (int j = 0; j < featureNumberPerSuperFeature_; j++) {
        memcpy(featuresBuffer + j * sizeof(uint64_t), &featureList[j], sizeof(uint64_t));
    }
    u_char superFeatureBuffer[SYSTEM_CIPHER_SIZE];
    cryptoObj_->generateHash(featuresBuffer, featureNumberPerSuperFeature_ * sizeof(uint64_t), superFeatureBuffer);
    string SFStr((char*)superFeatureBuffer, SYSTEM_CIPHER_SIZE);
    superFeatureList.push_back(SFStr);
    return true;
}
#elif SUPER_FEATURE_GEN_METHOD == MIN_FEATURE
bool Fingerprinter::extractSFList(uint64_t fpList[], int chunkSize, vector<string>& superFeatureList)
{
    vector<uint64_t> featureList;
    for (auto i = 0; i < keySeedFeatureNumber_; i++) {
    ...
    // cout << endl;
    for (int i = 0; i < featureList.size(); i += featureNumberPerSuperFeature_) {
        u_char featuresBuffer[SYSTEM_CIPHER_SIZE];
        memset(featuresBuffer, 0, SYSTEM_CIPHER_SIZE);
        for (int j = 0; j < featureNumberPerSuperFeature_; j++) {
            memcpy(featuresBuffer + j * sizeof(uint64_t), &featureList[i + j], sizeof(uint64_t));
        }
        u_char superFeatureBuffer[SYSTEM_CIPHER_SIZE];
        cryptoObj_->generateHash(featuresBuffer, featureNumberPerSuperFeature_ * sizeof(uint64_t), superFeatureBuffer);
        string SFStr((char*)superFeatureBuffer, SYSTEM_CIPHER_SIZE);
    ...
    sort(superFeatureList.begin(), superFeatureList.end());
    return true;
}
#elif SUPER_FEATURE_GEN_METHOD == ALL_FEATURE
bool Fingerprinter::extractSFList(uint64_t fpList[], int chunkSize, vector<string>& superFeatureList)
{
    vector<uint64_t> featureList;
    for (auto i = 0; i < keySeedFeatureNumber_; i++) {
    ...
    // cout << endl;
    u_char featuresBuffer[keySeedFeatureNumber_ * sizeof(uint64_t)];
    memset(featuresBuffer, 0, keySeedFeatureNumber_ * sizeof(uint64_t));
    for (int i = 0; i < featureList.size(); i++) {
        memcpy(featuresBuffer + i * sizeof(uint64_t), &featureList[i], sizeof(uint64_t));
    }
    u_char superFeatureBuffer[SYSTEM_CIPHER_SIZE];
    cryptoObj_->generateHash(featuresBuffer, keySeedFeatureNumber_ * sizeof(uint64_t), superFeatureBuffer);
    string SFStr((char*)superFeatureBuffer, SYSTEM_CIPHER_SIZE);
    superFeatureList.push_back(SFStr);
    return true;
}
