...
if(T){
    x1 <- read.table("upload_thread_2nd_line.data",header=TRUE)
    cairo_pdf(file="upload_thread_2nd_line.pdf",  width=mywidth, height=myheight)
    x1$type <- factor(x1$type, levels=c("SGX","DetectionFirst", "DetectionMin","DetectionAll"), labels=c("SGXDedup","SGXDedup+\n(firstFeature)","SGXDedup+\n(minFeature)","SGXDedup+\n(allFeature)"))
    ...
    theme(
      panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
	    ...
	    axis.text.x=element_text(margin=margin(7,0,0,0), angle=0, hjust=0.5, colour="black", size=24),
	    axis.title.y=element_text(size=23, hjust=0.9),
