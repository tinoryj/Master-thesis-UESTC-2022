...
if(T){
    ...
    x1$Netspeed <- factor(x1$Netspeed, levels=c("download"), labels=c("下载"))
    ...
    geom_text(aes(label = round(throughput,0)), position = position_dodge(0.8), hjust=0.5, vjust=-0.3,size=4,angle=0, color="black",family="Times New Roman") +
