...
if(T){
    ...
    geom_errorbar(aes(x=round,ymin=throughput-fraction, ymax=throughput+fraction), width=0.4, colour="black", position=position_dodge(0.9)) + 
    geom_text(aes(label = round(throughput,0)), position = position_dodge(0.90), hjust=0.5, vjust=-0.3,size=8,angle=0, color="black") +
