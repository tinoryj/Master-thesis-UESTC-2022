...
if(T){
    x1 <- read.table("trace_download.data",header=TRUE)
    cairo_pdf(file="trace_download.pdf", width=mywidth, height=myheight)
    x1$Type <- factor(x1$Type, levels=c("FSL", "MS", "Linux", "Couch"))
    ggplot(data=x1, aes(x=as.factor(BatchSize), y=Performance,shape=Type, linetype=Type, colour=Type, group=Type), log= "x") +
    geom_line(size=1.5)  + 
    # geom_point(size=1, stroke=1.5, fill="white") +
