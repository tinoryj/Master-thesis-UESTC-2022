...
myheight=0.8
if(T){
    x1 <- read.table("realBarPlot_legend.data", header=TRUE)
    cairo_pdf(file="realBarPlotSimilar_legend.pdf",  width=mywidth, height=myheight)
    x1$type <- factor(x1$type, levels=c("FirstF","MinF","AllF"), labels=c("1 Feature","2 Feature","3 Feature"))
    x1$groupType <- factor(x1$groupType, levels=c("similar", "feature","prefix-1" ,"prefix-2","prefix-3"), labels=c("Similarity","Feature" ,"Indicator-1" ,"Indicator-16","Indicator-32"))
