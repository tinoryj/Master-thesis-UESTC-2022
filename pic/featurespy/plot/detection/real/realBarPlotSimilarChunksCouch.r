...
if(T){
    ...
    x1 <- read.table("similarChunkCouch.data", header=TRUE)
    cairo_pdf(file="similarChunkCouch.pdf",  width=mywidth, height=myheight)
    ...
    coord_cartesian(ylim=c(0.9, 19)) +
    scale_y_continuous(breaks=seq(0, 18, 6), labels=format(seq(0, 18, 6), scientific=FALSE)) +
