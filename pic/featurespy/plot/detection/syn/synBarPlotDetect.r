...
if(T){
    ...
    geom_text(aes(label = round(ratio*100,0)), position = position_dodge(0.90), hjust=0.5, vjust=-0.3,size=7,angle=0, color="black") +
    ...
    theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
	    ...
	    axis.text.x=element_text(margin=margin(10,0,0,0), angle=0, hjust=0.5, colour="black", size=24),
        axis.title.x=element_text(margin=margin(5,-10,0,10), size=20),
	    axis.text.y=element_text(colour="black",size=26),
        axis.title.y=element_text(margin=margin(0, 10, 0, 0), size=26, hjust=0.5),
